...
class Mul_Ch_Wav_Mod_Sol(Frame):
    ...
    def __init__(self, master):
        ...
        self.vgrating_period = DoubleVar();self.vgrating_period.set(0.83);self.vdiffraction_mode = DoubleVar();self.vdiffraction_mode.set(1);
    ...
    def create_Entrys(self):
        ...
        self.filename = self.VaryH1LC_SaveAs.get();self.grating_period=float(self.grating_period_E.get());
        self.diffraction_mode=int(self.diffraction_mode_E.get());self.Nmodes_value=int(self.NModes_E.get())
...
####################################################################################################### mshu x y labela wash karw
    ...
    def NumOf_guided_modes(self): # This will print angles which can give coupled light.
        ...
        if self.g.NmaxPlan()==0: # blow H2==0.9 NmaxPlan is 0 and it does not give good value.
            ...
            for i in np.real((self.g.NumOf_guidedModes(Nmodes=self.Nmodes_value)[1:])):
                print("For the {} mode there is one coupling angle at {}".format(i,self.Nmode_effective(i)))
            # else:
    ...
    def Nmode_effective(self,n_eff): # to find n_e that is effective refractive index of the grating
        ...
        if ((n_eff-(self.diffraction_mode*(self.WL/self.grating_period)))/(n_top))>1:
            theta = 0
        elif ((n_eff-(self.diffraction_mode*(self.WL/self.grating_period)))/(n_top))<0:
            theta = 0
        else:
            theta=np.rad2deg(np.arcsin((n_eff-(self.diffraction_mode*(self.WL/self.grating_period)))/(n_top)))
